# AWS Lambda Triggered by S3 File Upload

This project demonstrates how to build an event-driven workflow where an AWS Lambda function is automatically triggered by file uploads to an Amazon S3 bucket.

## Architecture

* A private Amazon S3 bucket stores uploaded objects
* S3 is configured to send ObjectCreated event notifications
* An AWS Lambda function is triggered upon file upload
* The Lambda function:

  * Extracts the bucket name and object key from the event
  * Retrieves object metadata (e.g. content type) using S3 API
  * Logs relevant details to Amazon CloudWatch
...
1. Created a private S3 bucket and prepared sample files for upload
